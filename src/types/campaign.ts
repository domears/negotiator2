...
export interface Campaign {
  ...
  tertiaryKpis?: string[];
  goals?: Record<string, number>; // KPI name -> target value
  guardrails?: {
  ...
  deliverables: DeliverableRow[];
  planningMode: PlanningMode;
  setupProgress?: number; // e.g., 3 for 3/6 steps completed
  performanceScore?: number; // 0-100
  performanceLabel?: string; // "On track", "At risk", etc.
  rightsStatus?: 'ok' | 'attention';
  expiringRightsCount?: number;
  missingRightsCount?: number;
  overBudget?: boolean;
  underPacing?: boolean;
  pacingPercentage?: number; // e.g., -12 for 12% behind
  complianceConflicts?: boolean;
  blocked?: boolean;
  commentsCount?: number;
  filesCount?: number;
  lastEditedBy?: string;
  lastEditedAt?: Date;
  category?: string; // e.g., "Illustration", "Macro IG Reels"
  leadAvatar?: string;
  teamAvatars?: string[];
  createdAt: Date;
  updatedAt: Date;
